...
1.  **Перезагрузить** в длительном прессе \( Перезагружать \) и затем выбрать ![ пустой ][ImageRefreshIcon] **кэш и твердую перезагрузку.**  
...
1.  Выберите \. ![ Параметры ][ImageSettingsIcon] сети \) и включив почтовый ящик **скриншотов** захвата.
...
1.  Выберите \. Параметры сети \) снова и отключите контрольный ящик скриншотов захвата, чтобы закрыть области ![ ][ImageSettingsIcon] скриншотов. ****
...
1.  Выберите панель **ответа.**  Показан исходный код HTML.  
    ...
    > [!TIP]
    > При минировании файла выберите кнопку **Format** \( Format \) в нижней части панели ответа для повторного формата содержимого файла для ![ ][ImageFormatIcon] **** читаемости.  
...
1.  Выберите **Закрыть** \( Закрыть \) чтобы ![ снова ][ImageCloseIcon] просмотреть журнал сети.  
...
1.  Выберите **поиск** \. ![ Поиск ][ImageSearchIcon] \).  Поле Поиска открывается слева от журнала Network.  
...
1.  Выберите **фильтр** \( ![ Фильтр ][ImageFilterIcon] \) для его демонстрации.  
...
1.  Выберите **Добавить шаблон** \. Добавьте шаблон ![ ][ImageAddIcon] \).  
1.  Введите `main.css`.  
