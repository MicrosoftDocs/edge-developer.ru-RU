...
1.  Выберите **запись** \. ![ Значок запись ][ImageRecordIcon] \).  
...
1.  Выберите **страницу Обновления** \. ![ Страница обновления ][ImageRefreshPageIcon] \).  DevTools записывает показатели производительности при обновлении страницы, а затем автоматически останавливает запись через пару секунд после завершения нагрузки.  
...
Во время записи страницы выберите **Сбор** мусора \( Сбор значка мусора \) для ![ ][ImageCollectGarbageIcon] принудительного сбора мусора.  
...
Выберите **параметры** Capture \. Параметры захвата \) для выбора дополнительных параметров, связанных с тем, как ![ ][ImageCaptureSettingsIcon] DevTools фиксирует записи производительности.  
...
После записи выберите **clear recording** \( Clear recording icon \) для очистки этой записи с панели ![ ][ImageClearRecordingIcon] **Performance.**  
...
*   Используйте **кнопки Previous** \( ![ Previous ][ImagePreviousIcon] \) и **Next** ![ ][ImageNextIcon] \( Далее \).  
*   Выберите `Shift` + `Enter` для выбора предыдущего или `Enter` для выбора следующего.  

Изменение параметров запроса:  

*   Выберите **деликатный** \. ![ Деловая ][ImageSearchCaseIcon] чувствительная \) для того, чтобы сделать запрос чувствительным.  
*   Выберите **Regex** \( Regex \) для использования ![ ][ImageSearchRegexIcon] регулярного выражения в запросе.  
...
*   Выберите **режим** Pan \( Pan Mode \) для ![ перемещения по осям X и ][ImagePanModeIcon] Y.  
*   Выберите **режим вращать** \( ![ Вращать режим \) для ][ImageRotateModeIcon] поворота вдоль оси Z.  
*   Выберите **сброс преобразования** \. Сброс преобразования \) для ![ ][ImageResetTransformIcon] сброса схемы в исходное положение.  
