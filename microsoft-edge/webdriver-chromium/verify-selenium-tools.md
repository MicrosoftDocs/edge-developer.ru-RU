...
---
# Проверка загрузки средств Selenium для Microsoft Edge  

В этой статье приводится открытый ключ, который будет применяться при проверке подписи выпусков [средств Selenium для Microsoft Edge.][GithubMicrosoftEdgeSeleniumToolsReleases]  

[Java-выпуски][MavernSearchArtifactComMicrosoftEdgeMsedgeSeleniumToolsJava] [средств Selenium для Microsoft Edge подписаны][GithubMicrosoftEdgeSeleniumToolsReleases] с помощью [GPG.][Gnupg]  

Используйте следующие ключи для проверки подписи пакетов, предоставленных командой [microsoft Edge Developer Tools.][TwitterEdgeDevTools]  

## Открытый ключ для проверки средств Selenium для Microsoft Edge  

*   **ИД открытого ключа:** [](mailto:edgedevtoolsoss@microsoft.com) группа разработчиков Microsoft Edge EdgeDevToolsOSS@microsoft.com  
*   **Отпечаток открытого ключа:** `46EE EB3F 4028 B5CE A4E8  E6F5 A6DC D211 6D3A 3A7A`  
